...
<%@ page import="com.dreamsecurity.sso.server.config.*"%>
<%
...
<html>
<head>
<title>Magic SSO Server Install Test</title>
...
<body>
	<div id="content">
		<h1 align="center">Magic SSO Server Install Test : <%=SSOConfig.getInstance().getServerName()%></h1>
		<table>
			<thead>
				<tr>
...
%>
		...
		<table>
			...
			<tbody>
		    <tr>
				<td id='headkey'>IDP Session</td>
				<td id='headval'><%=session.getId()%></td>
			</tr>
<%
	...
	while (em.hasMoreElements()) {
		...
		if (skey.equals("AuthnRequest")) {
			out.println("<td id='headval'>" + URLEncoder.encode(((String) session.getAttribute(skey)).substring(0,60), "UTF-8") + "</td>");
		}
		else {
...
%>
		...
		<table>
			...
			<tbody>
				<tr>
					<td id='headkey'>Java Version</td>
					<td id='headval'><%=System.getProperty("java.version")%></td>
				</tr>
				<tr>
					<td id='headkey'>Java Endorsed path</td>
					<td id='headval'><%=System.getProperty("java.endorsed.dirs")%></td>
				</tr>
				<tr>
					<td id='headkey'>Dream home path</td>
					<td id='headval'>
<%
try{
...
%>		
				...
				<tr>
					<td id='headkey' valign="top">
						Server Type<br/>
						<input type="radio" name="type" value="IDP" checked="checked">IDP　
						<input type="button" id="loadingTestBt" name="loadingTestBt" value="class loading test" onclick="loadingTest(); return false;"/>
					</td>
					<td id='headval'>
						<div id="dumcon"></div>
						<iframe name="ifSrv1" id="ifSrv1" src="" scrolling="yes" frameborder="no" width="100%" height="200"></iframe>
						<iframe name="ifSrv2" id="ifSrv2" src="" scrolling="yes" frameborder="no" width="100%" height="450"></iframe>
					</td>
			    </tr>
			</tbody>
		</table>
