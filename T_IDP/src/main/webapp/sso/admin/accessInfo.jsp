...
<body>
	<jsp:include page="userPopup.jsp" flush="true"/>
	...
	<div class="page-holder">
		...
		<div class="page-header">
			<h4 class="title">접속 정보 조회</h4>
		</div>
		<div class="content-box">
			<div class="content-top">
				<div class="float-right">
					<input class="select-user width-200" type="text" id="sel-user" placeholder="전체 사용자" title="사용자 선택"/>
					<input class="date-input ml-10" type="text" id="sel-fdate" title="조회 시작일"/>
					<span class="date-between">&nbsp;∼&nbsp;</span>
					<input class="date-input" type="text" id="sel-tdate" title="조회 종료일"/>
					<select class="search-base width-120 ml-10" id="sel-type">
					    <option value="1" selected="selected">전체</option>
					    <option value="2">로그인</option>
					    <option value="3">로그아웃</option>
					    <option value="4">연계</option>
			...
			<div class="content-body pb-10" id="content">
...
<script type="text/javascript">
	...
	$(window).resize(function(){
		$('#reportGrid').setGridWidth($('#content').width(), true);
	});
	...
	$(function() {
		...
		$("#reportGrid").jqGrid({
			...
			colNames: ['No', '접속일', '접속 시간', '아이디', '이름', 'IP', '브라우저', '서버 ID', '접속 타입', '접속 결과'],
		    colModel: [
				{name:'index', index:'index', width:'5%', align:'center', sortable:false},
				{name:'logDate', index:'logDate', width:'12%', align:'center', sortable:false},
				{name:'logTime', index:'logTime', width:'12%', align:'center', sortable:false},
				{name:'userId', index:'userId', width:'10%', align:'center', sortable:false},
				{name:'userName', index:'userName', width:'10%', align:'center', sortable:false},
				{name:'accessIp', index:'accessIp', width:'12%', align:'center', sortable:false},
				{name:'accessBr', index:'accessBr', width:'12%', align:'center', sortable:false},
				{name:'accessSp', index:'accessSp', width:'15%', align:'center', sortable:false},
				{name:'accessType', index:'accessType', width:'15%', align:'center', sortable:false},
				{name:'accessRslt', index:'accessRslt', width:'17%', align:'left', sortable:false}
			],
			id: 'index',
		    rowNum: 10000000,
			gridview: true,
		...
		}).setGridWidth($('#content').width(), true);
	});
	...
	function search()
	{
		...
		***/
		...
		$("#reportGrid").setGridParam({ url:"sub/getAccessInfo.jsp", mtype:"POST", postData:{uid:uid, fdate:fdate, tdate:tdate, stype:stype, spage:1, pagerow:pagerow} });
	...
	function pageSearch(pageNo)
	{
		...
		***/
		...
		$("#reportGrid").setGridParam({ datatype:"json" });
		$("#reportGrid").setGridParam({ url:"sub/getAccessInfo.jsp", mtype:"POST", postData:{uid:uid, fdate:fdate, tdate:tdate, stype:stype, spage:pageNo, pagerow:pagerow} });
		$("#reportGrid").trigger("reloadGrid");
	}
	...
	function excel_search()
	{
		...
		***/
		...
		$.ajax({
			type: "POST",
			url: "sub/getExcelAccessInfo.jsp",
			data: {uid:uid, fdate:fdate, tdate:tdate, stype:stype},
			dataType: "JSON",
			async: false,
