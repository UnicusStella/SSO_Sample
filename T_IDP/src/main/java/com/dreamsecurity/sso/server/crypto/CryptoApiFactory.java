...
{
	private static Logger log = LoggerFactory.getLogger(CryptoApiFactory.class);
	...
	public static final String CLASS_MJC = "com.dreamsecurity.sso.server.crypto.api.MJCryptoApi";
	public static final String CLASS_JCAOS = "com.dreamsecurity.sso.server.crypto.api.JCAOSCryptoApi";
	...
	public static void setInitCryptoAuditInfo()
	{
		for (int i = 0; i < auditList.size(); i++) {
			...
			Util.setAuditInfo(audit.getDate(), audit.getTime(), audit.getUser(), audit.getType(), audit.getResult(), audit.getDetail());
		}
	}
		...
		try {
			...
			if (OS.indexOf("win") >= 0) {
				...
				if (!Util.isEmpty(readline)) {
					log.error("### Magic SSO Server Password Mismatch.");
