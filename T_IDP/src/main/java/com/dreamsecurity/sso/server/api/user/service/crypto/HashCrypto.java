package com.dreamsecurity.sso.server.api.user.service.crypto;
...
public class HashCrypto
{
	...
	private static HashCrypto instance;
	...
	{
		if (instance == null) {
			synchronized (HashCrypto.class) {
				if (instance == null) {
					instance = new HashCrypto();
				}
			}
		}
	...
	public String getHash(String src)
	{
		try {
			...
			log.error("### getHash(1) Exception: {}", e.toString());
		}
	...
	public String getHashWithSalt(String src, String salt, String hashAlgo, int charCode)
	{
		try {
			byte[] hashedSalt = CryptoApiFactory.getCryptoApi().digest(salt.getBytes("UTF-8"), hashAlgo);
			String newSrc = src + new String(hashedSalt);
	...
	public String getSha256Base64(String src, String salt) throws Exception
	{
		try {
			byte[] hashedSalt = CryptoApiFactory.getCryptoApi().digest(salt.getBytes("UTF-8"), _SHA256);
			String newSrc = src + new String(hashedSalt);

			byte[] hasedB = CryptoApiFactory.getCryptoApi().digest(newSrc.getBytes("UTF-8"), _SHA256);
			return Util.encode64(hasedB);
		}
		catch (Exception e) {
			log.error("### getSha256Base64() Exception: {}", e.toString());
	...
	public String getSha256Hex(String src, String salt) throws Exception
	{
		try {
			byte[] hashedSalt = CryptoApiFactory.getCryptoApi().digest(salt.getBytes("UTF-8"), _SHA256);
			String newSrc = src + new String(hashedSalt);
