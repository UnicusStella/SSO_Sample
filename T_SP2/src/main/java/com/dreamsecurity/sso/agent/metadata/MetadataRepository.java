...
{
	private static Logger log = LoggerFactory.getInstance().getLogger(MetadataRepository.class);
	...
	public static MetadataRepository getInstance()
	{
		...
		loadMetadata(config.getHomePath(config.getMetadataPath()));
		...
		try {
			...
			synchronized (instance) {
				...
				for (int i = 0; i < spEntities.size(); i++) {
					...
					// log.debug(spEntity.getEntityID() + " meta loaded");
	...
	protected EntityDescriptor getEntityDescriptor(String entityId) throws SSOException
	{
		...
		// todo reload
		log.error("### No matched entity descriptor [" + entityId + "]");
